...
export function toRheaMessage(msg, encoder) {
    ...
    if (amqpMsg.ttl != null && amqpMsg.ttl !== Constants.maxDurationValue) {
        ...
        amqpMsg.absolute_expiry_time = new Date(Math.min(Constants.maxAbsoluteExpiryTime, amqpMsg.creation_time.getTime() + amqpMsg.ttl));
    }
    if (isAmqpAnnotatedMessage(msg)) {
        return amqpMsg;
    }
...
export function fromRheaMessage(rheaMessage, options) {
    ...
    if (rheaMessage.ttl == null)
        rheaMessage.ttl = Constants.maxDurationValue;
